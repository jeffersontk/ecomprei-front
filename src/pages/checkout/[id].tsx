...
export default function Checkout({ product, copy }: CheckoutProps) {
  ...
  const paragraphs = copy[0]?.paragraphs.reverse() ?? []
  const copyImagesList = product.variantsImage
  const copyWithTextAndImageLink = product.videoUrl
    ? alternateArrays(paragraphs, copyImagesList)
