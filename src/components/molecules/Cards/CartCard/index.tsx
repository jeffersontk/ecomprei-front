...
import { BoxImage } from '../../../../styles/pages/Cart'
...
export default function CartCard({
  ...
  function calcularDesconto(
    ...
    const result = precoTotal - precoTotal * (porcentagemDesconto / 100)
    return +result
  }
        ...
        >
          <Box display="flex" alignItems="flex-start" gap="2">
            {colors && colors.length > 0 && (
              ...
              >
                <FormLabel htmlFor="#" margin={0} mr="1">
                  Cor
                </FormLabel>
                <Select
                  height={'6'}
                  value={
                    newColorSelect.length === 0 ? colorSelect : newColorSelect
                  }
                  onChange={(e) => setNewColorSelect(e.target.value)}
                >
                  {colors.map((item) => {
                    if (item.variant) {
                      return (
            ...
            {sizes && sizes.length > 0 && (
              ...
              >
                ...
                <Select
                  height={'6'}
                  value={
                    newSizeSelect.length === 0 ? sizeSelect : newSizeSelect
                  }
                  onChange={(e) => setNewSizeSelect(e.target.value)}
                >
                  {sizes.map((item) => {
                    if (item.size) {
                      return (
          ...
          <Box display="flex" alignItems="center" gap="2">
            <IconButton
              ...
              aria-label=""
              icon={
                quantity > 1 ? <BiMinus size={14} /> : <BiTrash size={14} />
              }
